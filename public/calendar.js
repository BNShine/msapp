    ...
    async function loadInitialData() {
        ...
        try {
            ...
            if (!techDataResponse.ok) {
                ...
                throw new Error(`Failed to load technician list (Status: ${techDataResponse.status}). Response: ${errorText.substring(0, 50)}...`);
            }
            if (!appointmentsResponse.ok) {
                const errorText = await appointmentsResponse.text();
                throw new Error(`Failed to load appointments list (Status: ${appointmentsResponse.status}). Response: ${errorText.substring(0, 50)}...`);
            }
